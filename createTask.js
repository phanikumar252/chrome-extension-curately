...
let innerHtmlOfPoup =
  `
    <div id="processing_popup" style="display: none;box-shadow: 3px 5px 25px -2px;position: fixed;width: 300px;background: white;bottom: 0;left: 0;padding: 10px;z-index: 9999999999999999999;height: 325px;border-radius: 10px;">
      ...
      <div style="margin-top: 10px;">
        <ul style="max-height: 270px;overflow: auto;padding: 10px;" id="progress_list"></ul>
      </div>
    </div>
...
section.setAttribute('class', 'curatly-section l-full-height');
...
document.getElementById('play_processing').addEventListener('click', () => {
  ...
  letsAutomationStartProcess(listOfUrlJson, tsakToCreate, messageToSend, true);
});
    ...
    for (let index = 0; index < newValue.length; index++) {
      ...
      if (container.querySelector(`[id="${newValue[index].src}loader"]`) != null) {
        if (newValue[index].hasError) {
          ...
          container.querySelector(`[id="${newValue[index].src}loader"]`).innerHTML = stopedIcon;
          container.querySelector(`[id="${newValue[index].src}error"]`).textContent = "stoped";
        }
        continue;
      }
      ...
      startDiv.textContent = newValue[index].name;
